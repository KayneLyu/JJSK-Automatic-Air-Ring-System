...
for (const dsName of ['01', '02', '03', '04', '05'] as const) {
  test(`诊断: 样本数据 ${dsName} 的片段 NaN 比例和脉冲范围`, async () => {
    ...
    for (let i = 0; i < tripSegment.length; i++) {
      ...
      const withPulse = seg.measurements.filter(m => m.pulse !== undefined).length
      const pulsesInBounds = seg.measurements.filter(m => !isNaN(m.y) && m.pulse !== undefined)
      const pulseMin = pulsesInBounds.length > 0 ? Math.min(...pulsesInBounds.map(m => m.pulse!)) : NaN
      const pulseMax = pulsesInBounds.length > 0 ? Math.max(...pulsesInBounds.map(m => m.pulse!)) : NaN
      const yValues = seg.measurements.filter(m => !isNaN(m.y)).map(m => m.y)
      const yMean = yValues.length > 0 ? yValues.reduce((a, b) => a + b, 0) / yValues.length : NaN
      const yStd = yValues.length > 0 ? Math.sqrt(yValues.reduce((a, b) => a + (b - yMean) ** 2, 0) / yValues.length) : NaN
      console.log(`  Seg${i}: total=${total} nan=${nanCount}(${(nanCount/total*100).toFixed(0)}%) inBounds=${inBounds} dur=${(seg.duration/1000).toFixed(0)}s fwd=${seg.isForward}`)
      console.log(`    pulseRange=[${pulseMin?.toFixed(0)},${pulseMax?.toFixed(0)}] (range=${(pulseMax-pulseMin)?.toFixed(0)}) yMean=${yMean?.toFixed(0)} yStd=${yStd?.toFixed(0)} DC/AC=${(yMean/yStd)?.toFixed(1)}`)
    }

    const maxAngle = estimateThetaMaxWithPhaseCorrection(tripSegment) ?? 0
    const error = Math.abs(info.angle - maxAngle)
    console.log(`  Result: expected=${info.angle}° got=${maxAngle.toFixed(2)}° error=${error.toFixed(2)}°`)
