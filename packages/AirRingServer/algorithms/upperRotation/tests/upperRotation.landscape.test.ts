...
type Seg = {
  data: { t: number; y: number; offsetDeg: number }[]
  duration: number
  accelRatio: number
}

function evaluatePulseExpanded(
  segs: Seg[],
  thetaDeg: number,
  NUM_BINS = 36
): number {
  ...
  for (const { data, duration, accelRatio } of segs) {
    for (const p of data) {
      ...
      const phi =
        trapezoidalPosition(p.t / duration, accelRatio) * thetaRad +
...
for (const dsName of ['01', '02', '04'] as const) {
  test(`loss landscape: 样本数据 ${dsName}`, async () => {
    ...
    for (let i = 0; i < upper.length; i++) {
      const u = upper[i]
      const t = thicknessData[i]
      if (u && t)
        segs = btsNext({ airRing: u, thickness: { ...rollerNext(), ...t } })
    }
    ...
    const fullSegs = segs.filter(
      (s) => s.duration >= minDur && s.measurements.length >= 10
    )
    const useSegs =
      fullSegs.length >= 2
        ? fullSegs
        : segs.filter((s) => s.measurements.length > 0)
    ...
    for (const seg of useSegs) {
      ...
      pulseSegs.push({
        ...
        accelRatio: accelMs / seg.duration,
      })
    }
    ...
    console.log(
      `pulseRange: [${pulseMin.toFixed(0)}, ${pulseMax.toFixed(0)}] = ${pulseRange.toFixed(0)}`
    )

    // Find minima
    let bestPulse = Infinity,
      bestPulseTh = 0
    let bestOrig = Infinity,
      bestOrigTh = 0
        ...
        th === Math.round(expected)
          ...
          : th === bestPulseTh
            ...
            : th === bestOrigTh
              ...
              : ''
      console.log(
        `  θ=${th}°: pulse=${lp.toFixed(6)} orig=${lo.toFixed(6)}${marker}`
      )
    }

    console.log(
      `PULSE best: θ=${bestPulseTh}° (error ${Math.abs(bestPulseTh - expected).toFixed(1)}°)`
    )
    console.log(
      `ORIG  best: θ=${bestOrigTh}° (error ${Math.abs(bestOrigTh - expected).toFixed(1)}°)`
    )
