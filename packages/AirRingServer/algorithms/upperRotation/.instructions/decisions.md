...
## D-028 机器适配优先使用 adaptiveTuning（少参数）

- 状态：active
- 日期：2026-04-03
- 选择：
  - 对外优先暴露 `adaptiveTuning`（5 个高层参数）作为机台/配方适配接口；
  - `adaptiveRules` 仅保留为高级调试/专家覆盖通道。
- 理由：
  - 直接暴露完整规则树参数过多，不适合现场工程调参与运维；
  - 高层参数可覆盖主要调节方向（低角度融合、质量严格度、高角度窗口偏置、数据量门控）。
- 结果：
  - 适配流程更可操作，减少误配风险；
  - 在默认 tuning 下保持现有回归结果（真实集 5/5 通过）。
