...
class SignalSimulator {
  ...
  public updateOneStep(deltaTimeMs: number = 10, currAngle: number = 0): void {
    ...
    // 探头值：sin波动模拟厚度
    this.signals.probeValue = Math.floor(
      800 + 100 * Math.sin((currAngle / 180) * Math.PI)
    )
    ...
    this.signals.rotationPulse += Math.floor(
      (deltaTimeMs / 1000) *
        ((this.params.angleVelocity * 60) / 360) *
        pulsePerDegree
    )
    ...
    // 换向：限位触发
    this.signals.swapDirection =
      this.signals.leftLimit || this.signals.rightLimit
  ...
  public startRealtime(durationMs: number = 10000): NodeJS.Timeout {
    ...
    const intervalId = setInterval(() => {
      ...
      // 简单角度更新
      simAngle =
        (simAngle + (this.params.angleVelocity / 6) * (deltaTime / 1000)) % 360
