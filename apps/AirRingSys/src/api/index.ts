import { formateResult, postRequest } from '@/utils/axios.ts'
...
export const getAirRingWarningList = () => {
  return formateResult<IWarningList[]>(
    postRequest('/api/airRing/getWarningList')
  )
}
...
export const UploadThickness = () => {
  return formateResult<{ D: Array<number | string> }>(
    postRequest('/api/thk/getTempFrame')
  )
}
