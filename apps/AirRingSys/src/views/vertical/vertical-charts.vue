...
const { updateCharts, selectSeriesIndex } = useChartsInit(
  'chartContainer',
  option,
  props
)
...
watch(
  ...
  (newValue) => {
    ...
    // 计算平均值
    const sigmaMeanValue =
      sigmaCount === 0 ? 0 : Number((sigmaSum / sigmaCount).toFixed(1))
...
<template>
  <div class="charts">
    ...
    <div class="show_text title_left">
      <div>
        <p>
          <span style="font-size: 16px">{{ $t('vertical.sigma2') }}</span>
    ...
    <div class="show_text title_right">
      ...
      <p>
        <span>{{ `${$t('vertical.max')}: ${sigmaInfo.sigmaMax}%` }}</span>
        <span>{{
          `${$t('vertical.min')}:
                    ${sigmaInfo.sigmaMin}%`
        }}</span>
