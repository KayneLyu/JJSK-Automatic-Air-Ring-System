...
function useOperateChartsHooks() {
  ...
  const getTrendDataList = async (pickDate?: string) => {
    ...
    try {
      ...
      console.warn('[useOperateCharts] getTrendDataList failed')
    }
  }
  ...
  const nextPageQuery = async (isBack: boolean) => {
    ...
    if (isBack) {
      ...
      startId =
        startNumbers.frameId - store.queryHours * 100 < 0
          ? 0
          : startNumbers.frameId - store.queryHours * 100
    ...
    try {
      ...
      console.warn('[OperateCharts] 查询失败')
    }
  }
  ...
  const changeStep = (step: number) => {
    ...
    let index = queryDataList.value.findIndex(
      (item) => item.frameId === currentId.value
    )
    index += step
