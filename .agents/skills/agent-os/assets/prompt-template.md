...
- 必须包含三个部分：
  1. **项目结构概览**（3–5 行）
  2. **开始工作前**（强制执行）：声明必须先读取 `README.md` 和 `.agents/README.md`
  3. **核心规则摘要**（5–10 行）
- AGENTS.md 的角色是**入口路由**：告诉 Agent 先读什么、核心规则是什么、去哪里找更详细的约束
- 不写完整规则（规则在 `.agents/guide/`）
- 不写行为契约表（契约在 `.agents/README.md`）
- 已存在则增量更新，禁止无差别重写
...
更新规则：仅补充缺失文件；已存在文件保留用户内容，禁止无差别重写；创建 `scripts/`、`scripts/outputs/` 如不存在。

`.agents/README.md` 必须逐项检查并补齐（每次调用都做，不仅是不存在时创建）：
- 检查是否有目录结构概览（树形）→ 若无则追加
- 检查是否有**Agent 行为契约表**（场景→必须读取→核心约束，路径相对于 `.agents/` 根目录）→ 若无则在目录结构之后追加
- 检查行为契约表中是否有**"不读 = 违规"**约束声明 → 若无则追加
- 检查是否有 `scripts/` 与 `outputs/` 使用规范 → 若无则追加
- 检查是否有 Script First 最佳实践 → 若无则追加
- 检查底部集成说明是否为旧版 "Navigation: Start from AGENTS.md" → 若是则替换为路由说明：`AGENTS.md` 是入口路由器；本 `README.md` 是行为契约与基础设施说明的核心枢纽
- 不修改用户手写内容、不翻译已有段落

`README.md` 存在性检查：
- 检查项目根目录是否存在 `README.md`；若不存在，在输出中标记 `⚠️  提示: 项目缺少 README.md，建议创建以帮助 Agent 理解项目目标`
- **禁止修改项目的 README.md**
...
**首要原则：guide 文件的每一段都必须是对 Agent 行为的可执行约束，而非描述。** 如果一段文字只是"介绍"而不产生行为限制，应删除或改写。
...
- **时间必须来自系统时间命令（`date "+%Y-%m-%d %H:%M"` 或等效方式），禁止编造时间**
- `decisions.md` 新记录必须插入在文件顶部（时间倒序，最新在前）；`progress.md` 新记录必须追加在文件底部（时间正序，只追加）；所有记录禁止时间乱序
